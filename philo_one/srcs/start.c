...
/*   Updated: 2020/09/18 13:30:18 by cbertola         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_messages_dead(t_philo *philo, long int time, t_gbl *gbl, char *message)
{
    ...
    // {
        aff_msg(get_time(philo->t_die), philo->id + 1, "IS DEAD DIFF", 1);
...
void        *ft_start(void *args)
{
	...
	ft_messages(philo, get_time(gbl->t_start), gbl, "was created");
	philo->t_die = get_time(0);
